...
#' @param lmf_ext A string specifying the custom pattern for tree top files. Default is 'lmf_ws_{window_size}'.
#'
...
approximate_chm_crowns <- function(proj_dir,
  ...
  # If CHM is very large, load to disk
  ...
  if('fixed' %in% crown_methods) {
    ...
    if (is.null(lmf_ext)) {
      lmf_ext <- glue::glue('lmf_ws_{window_size}')
    }
    ...
    # Load Tree Tops
    ttops_ws <- sf::st_read(stringr::str_subset(ttops_files, pattern = lmf_ext), quiet = TRUE)

    # If treetops don't have a treeID column make one using rowid_to_column
    if(!'treeID' %in% names(ttops_ws)) {
      ttops_ws <- tibble::rowid_to_column(ttops_ws, 'treeID')
    }
